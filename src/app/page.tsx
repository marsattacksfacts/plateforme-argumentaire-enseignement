...
export default function Home() {
  ...
  useEffect(() => {
    supabase.from("inscriptions").select("*", { count: "exact", head: true }).then(({ count }) => setNbInscrits(count || 0));
    supabase.from("config").select("value").eq("key", "nb_lettres").single().then(({ data }) => { if (data) setNbLettres(parseInt(data.value) || 0); });
  }, []);
    ...
    <main className="min-h-screen bg-[#F5F0E8] text-[#1C1917] font-sans">
      ...
      <div className="bg-[#F5F0E8] px-4 md:px-8 pb-8">
        <div className="max-w-5xl mx-auto flex flex-wrap gap-3">
          <a href="/traces/jour1.gpx" download className="border border-[#C0440E] text-[#C0440E] font-medium px-5 py-2.5 text-sm hover:bg-[#C0440E]/5 transition-colors inline-flex items-center gap-2">🗺️ Trace — Jour 1</a>
          <a href="/traces/jour2.gpx" download className="border border-[#C0440E] text-[#C0440E] font-medium px-5 py-2.5 text-sm hover:bg-[#C0440E]/5 transition-colors inline-flex items-center gap-2">🗺️ Trace — Jour 2</a>
          <a href="/traces/jour3.gpx" download className="border border-[#C0440E] text-[#C0440E] font-medium px-5 py-2.5 text-sm hover:bg-[#C0440E]/5 transition-colors inline-flex items-center gap-2">🗺️ Trace — Jour 3</a>
        </div>
      </div>
      ...
      <section id="parcours" className="bg-[#1C1917] text-[#F5F0E8] px-4 md:px-8 py-16">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div><p className="text-[10px] uppercase tracking-widest text-[#C0440E] mb-3">Le parcours</p><h2 className="font-serif text-3xl font-bold">{showTroncons ? `${troncons.length} tronçons, ${Math.round(totalKm)} km` : `${haltes.length} étapes`}</h2></div>
            <button onClick={() => setShowTroncons(!showTroncons)} className="text-xs text-[#F5F0E8]/60 hover:text-[#F5F0E8] border border-[#F5F0E8]/30 px-3 py-1.5 transition-colors">{showTroncons ? "Voir les étapes" : "Voir les tronçons"}</button>
          </div>
          {!showTroncons ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-px bg-white/10">
              {haltes.map((h) => {
                const isNuit = h.type === "nuit", isEtapeCle = h.type === "etape_cle", isDepart = h.type === "depart", isArrivee = h.type === "arrivee";
                const isKey = isNuit || isEtapeCle || isDepart || isArrivee;
                const hasDetails = h.lieu || h.adresse || h.notes || h.heure_rassemblement;
                return (
                  <div key={h.id} className={`group relative ${hasDetails ? "cursor-pointer" : ""} ${isNuit ? "bg-[#E8B43A]/15 hover:bg-[#E8B43A]/25" : isEtapeCle ? "bg-[#C0440E]/20 hover:bg-[#C0440E]/30" : isDepart ? "bg-[#C0440E]/20 hover:bg-[#C0440E]/30" : isArrivee ? "bg-[#C0440E]/20 hover:bg-[#C0440E]/30" : "bg-[#1C1917] hover:bg-[#2a2420]"} transition-colors duration-200`}>
                    <div className="p-4" onClick={() => hasDetails && setExpandedHalte(expandedHalte === h.id ? null : h.id)}>
                      <p className={`font-serif text-2xl font-black leading-none mb-1 ${isKey ? "text-[#C0440E]" : "text-white/15"}`}>{String(h.ordre).padStart(2, "0")}</p>
                      <p className="font-medium text-sm text-[#F5F0E8]">{h.ville}</p>
                      <p className={`text-[10px] uppercase tracking-widest mt-0.5 ${isNuit ? "text-[#E8B43A]" : isEtapeCle ? "text-[#C0440E]" : isDepart ? "text-[#C0440E] opacity-70" : isArrivee ? "text-[#C0440E] opacity-70" : "text-[#F5F0E8] opacity-40"}`}>{isNuit ? `🌙 Nuit J${h.jour}` : isEtapeCle ? `🍽️ Étape clé` : isDepart ? "Départ" : isArrivee ? "Arrivée" : TYPE_LABELS[h.type] || h.type}</p>
                      {isNuit && <span className="absolute top-2 right-2 text-xs">🌙</span>}
                      {isEtapeCle && <span className="absolute top-2 right-2 text-xs">🍽️</span>}
                    </div>
                    {hasDetails && (
                      <div className={`absolute left-0 right-0 top-full bg-[#1C1917] border border-[#C0440E]/30 p-3 shadow-2xl z-30 text-[#F5F0E8] text-xs space-y-1 ${expandedHalte === h.id ? "block" : "hidden group-hover:block"}`}>
                        ...
                        {h.type === "nuit" && h.heure_rassemblement_depart && <p>🕐 Rassemblement départ : {h.heure_rassemblement_depart}</p>}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="text-left text-[#F5F0E8]/60 text-[10px] uppercase tracking-widest"><th className="p-3">#</th><th className="p-3">Départ</th><th className="p-3">Arrivée</th><th className="p-3 text-right">Distance</th><th className="p-3 text-right">D+</th></tr></thead>
                <tbody>
                  {troncons.map((t) => { const from = haltes.find(h => h.id === t.halte_depart_id), to = haltes.find(h => h.id === t.halte_arrivee_id); return (<tr key={t.id} className="border-t border-white/10 hover:bg-white/5 transition-colors"><td className="p-3 text-[#C0440E] font-bold">{t.ordre}</td><td className="p-3">{from?.ville || "—"}</td><td className="p-3">{to?.ville || "—"}</td><td className="p-3 text-right font-medium">{t.distance_km} km</td><td className="p-3 text-right text-[#C0440E]">+{t.denivele_pos}m</td></tr>); })}
                  <tr className="border-t-2 border-[#C0440E] font-bold"><td className="p-3"></td><td className="p-3" colSpan={2}>Total</td><td className="p-3 text-right">{Math.round(totalKm)} km</td><td className="p-3 text-right text-[#C0440E]">+{totalDplus}m</td></tr>
                </tbody>
              </table>
            </div>
          )}
      ...
      <footer className="bg-[#1C1917] border-t border-white/10 px-4 md:px-8 py-6 text-xs text-[#F5F0E8]/50">
        <div className="max-w-5xl mx-auto flex flex-wrap justify-between gap-3">
          ...
          <p className="text-right"><Link href="/code-ethique" className="underline hover:text-[#F5F0E8]/80 transition-colors">Code éthique</Link> · <Link href="/confidentialite" className="underline hover:text-[#F5F0E8]/80 transition-colors">Politique de confidentialité</Link><br /><Link href="/admin/login" className="text-[#F5F0E8]/30 hover:text-[#F5F0E8]/50 transition-colors">Admin</Link> · <Link href="/hebergement/login" className="text-[#F5F0E8]/30 hover:text-[#F5F0E8]/50 transition-colors">Hébergement</Link></p>
        </div>
      </footer>
    </main>
  );
