...
function cumulKmAtIdx(idx: number): number {
  let d = 0;
  for (let i = 1; i <= idx && i < TRACE.length; i++) d += distM(TRACE[i-1][0], TRACE[i-1][1], TRACE[i][0], TRACE[i][1]);
  return d / 1000;
}
...
function computeRolling(locs: { lat: number; lng: number; created_at: string }[]) {
  ...
  for (let i = 1; i < locs.length; i++) {
    ...
    if (d > 30 && dt < 300) { totalSec += dt; totalKm += d / 1000; }
  }
...
const kmBornes: Record<1 | 2 | 3, [number, number]> = { 1: [0, 68], 2: [63, 120], 3: [115, 170] };
...
export default function LivePage() {
  ...
  for (let i = 0; i < HALTES.length - 1; i++) {
    if (HALTES[i].kmTrace <= distParcourueKm && HALTES[i + 1].kmTrace > distParcourueKm) { idxHalteDepart = i; break; }
  }
  const kmDebutTroncon = HALTES[idxHalteDepart].kmTrace;
  const distSurTronconKm = Math.max(0, distParcourueKm - kmDebutTroncon);
  const locsSurTroncon = locations.filter(loc => cumulKmAtIdx(closestTraceIdx(loc.lat, loc.lng)) >= kmDebutTroncon);
  ...
  const vitesseEstim = distSurTronconKm > 1 ? (vitesseTroncon > 0 ? vitesseTroncon : vitesseGenerale || 12) : (vitesseGenerale > 0 ? vitesseGenerale : 12);
  ...
  TRACE.forEach((_, i) => { const km = cumulKmAtIdx(i); if (km >= kmMin && km <= kmMax) traceJourIndices.push(i); });
  ...
  const project = (lat: number, lng: number): { x: number; y: number } => {
    ...
    const lats = traceJour.map(t => t[0]), lngs = traceJour.map(t => t[1]);
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4 bg-[#F5F0E8]/90 backdrop-blur border-b border-black/10">
        ...
        <div className="flex gap-2">
          {([1, 2, 3] as const).map(j => (
            <button key={j} onClick={() => { setCarteJour(j); setAutoJour(false); }} className={`text-xs px-2 py-1 border transition-colors ${displayJour === j ? "bg-[#C0440E] text-white border-[#C0440E]" : "border-black/10 text-[#6B6459] hover:border-black/30"}`}>J{j}</button>
          ))}
        </div>
      </nav>
      ...
      <section className="pt-28 pb-20 px-4 md:px-8 max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            ...
            <div key={s.label} className="bg-white border border-black/10 p-4 text-center"><p className="font-serif text-3xl font-black text-[#C0440E]">{s.value}</p><p className="text-[10px] text-[#6B6459] uppercase tracking-wide mt-1">{s.label}</p></div>
          ))}
        </div>

        {prochaineHalte && (
          <div className={`p-6 text-center mb-10 border ${kmRestant < 0.8 ? "bg-green-50 border-green-300" : "bg-[#C0440E]/5 border-[#C0440E]/20"}`}>
            {kmRestant < 0.8 ? (
              <>
                ...
                {(() => {
                  ...
                  return (
                    <div className="mt-3 pt-3 border-t border-green-200">
                      ...
                      {minToNext > 0 && <p className="text-lg font-bold text-[#C0440E] mt-1">~{Math.round(minToNext)} min · Arrivée vers {new Date(Date.now() + minToNext * 60000).toLocaleTimeString("fr-BE", { hour: "2-digit", minute: "2-digit" })}</p>}
                    </div>
                  );
                })()}
              </>
              ...
              <>
                ...
                {tempsRestantMin > 0 && <p className="text-lg font-bold text-[#C0440E] mt-2">~{Math.round(tempsRestantMin)} min · Arrivée vers {new Date(Date.now() + tempsRestantMin * 60000).toLocaleTimeString("fr-BE", { hour: "2-digit", minute: "2-digit" })}</p>}
              </>
            )}
          </div>
        )}
        ...
        <div className="border border-black/10 bg-[#FBF6ED] overflow-hidden">
          <svg viewBox="0 0 800 300" className="w-full" style={{ height: "auto" }}>
            {traceJour.length > 1 && <polyline points={traceJour.map(t => { const {x,y} = project(t[0],t[1]); return `${x},${y}`; }).join(" ")} fill="none" stroke="#C0440E" strokeWidth="2" strokeDasharray="6 4" opacity="0.35" />}
            {traceDone.length > 1 && <polyline points={traceDone.map(t => { const {x,y} = project(t[0],t[1]); return `${x},${y}`; }).join(" ")} fill="none" stroke="#C0440E" strokeWidth="3" strokeLinecap="round" />}
            {haltesJour.map(h => { const {x,y}=project(h.lat,h.lng); const s=getHalteStyle(h.type); return <g key={h.id}><circle cx={x} cy={y} r={s.r} fill={s.fill} stroke={s.stroke} strokeWidth="1.5" />{h.type==="nuit"&&<text x={x} y={y+4} textAnchor="middle" fontSize="6" fill="#1C1917">🌙</text>}<text x={x} y={y>60?y-s.r-4:y+s.r+10} textAnchor="middle" fontSize="8" fill="#1C1917" fontWeight="600" fontFamily="sans-serif">{h.ville}</text></g>; })}
            {showPosition && <g><circle cx={posActuelle.x} cy={posActuelle.y} r="9" fill="#22c55e" opacity="0.25" /><circle cx={posActuelle.x} cy={posActuelle.y} r="6" fill="#22c55e" stroke="white" strokeWidth="2" /></g>}
          </svg>
        </div>
